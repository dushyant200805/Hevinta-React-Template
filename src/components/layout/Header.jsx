...
import { faBars, faMoon, faSun, faX, faXmark } from "@fortawesome/free-solid-svg-icons";
function Header() {
  // offcanvas
  const handleClose = () => {
  ...
  return (
    // <!-- header -->
    <header className={`position-absolute w-100 ${isFixed ? "fix" : ""}`}>
      <nav className="navbar navbar-expand-lg">
        <div className="container">
          ...
          <div className="offcanvas offcanvas-end" id="offcanvasNavbar2">
            ...
            <div className="offcanvas-body">
              <ul className="navbar-nav justify-content-center flex-grow-1 pe-3 gap-lg-3" id="nav_ul">
                ...
                <li className={`nav-item ${pathname === "/about" ? "active" : ""}`} onClick={handleClose}>
                  <Link to="/about" className="nav-link text-uppercase fw-semibold" >
                    About
                  </Link>
                </li>
                <li className={`nav-item ${pathname === "/causes" ? "active" : ""}`} onClick={handleClose}>
                  <Link to="/causes" className="nav-link text-uppercase fw-semibold">
                    Causes
                  </Link>
                </li>
                <li className={`nav-item ${pathname === "/events" ? "active" : ""}`} onClick={handleClose}>
          ...
          <div className="d-flex gap-2">
            {/* <!-- Mobile Menu Button --> */}
            <button className="navbar-toggler btn prime-outline-btn2 rounded-0 shadow-none p-2" type="button" data-bs-target="#offcanvasNavbar2" data-bs-toggle="offcanvas" aria-controls="offcanvasNavbar2" aria-label="Toggle navigation">
              <FontAwesomeIcon icon={faBars} className="fs-5 prime-text" />
            </button>
            <button id="btnSwitch" className="btn prime-btn theme-btn border rounded-0" aria-label="theme-switch button" onClick={toggleTheme}>
              {theme === "light" ? (
